...
function executive(results: ScanResult[], score: number, coverage: number): ScanReport["executive"] {
  ...
  const verdict = score >= 90 ? `Strong posture across tested controls.${coverageWarning}` : score >= 75 ? `Good posture with remediation required before high-assurance release.${coverageWarning}` : score >= 60 ? `Moderate risk; several weaknesses should be addressed before production confidence.${coverageWarning}` : `Elevated risk; immediate remediation and retesting are recommended.${coverageWarning}`;
...
export function printReport(report: ScanReport, noColor = false): void {
  ...
  for (const r of sorted) {
    ...
    console.log(`  ${icon} ${bold}${r.title}${reset} ${color}[${r.severity.toUpperCase()}]${reset} ${gray}${resultKind(r)}${reset} ${gray}${r.confidence}${reset} ${gray}${r.exploitability ?? "theoretical"}${reset}`);
    if (r.status !== "pass") {
